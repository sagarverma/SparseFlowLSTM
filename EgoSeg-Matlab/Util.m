classdef Util < handle
   ...
   methods(Static)
        ...
        function PrepreocessSequences(fname_mask,suffix,labelmap_mat_filename)
            ...
            % string.
            %
            % labelmap_mat_filename - a .mat file with a variable
            % called 'labelmap' of type cell array, with size 2XN
            % where N is the number of labels in the dataset. 
            % Each label can be re-mapped using this variable. 
            % For example, labels that represent activities which we would
            % like to ignore could be remapped to the 'DontCare' label. If
            % this argument is empty (0x0 mat) than no mapping is
            % performed.
            ...
            for i=1:numel(fullnames)
                ...
                try
                    ...
                    % read elan:
                    fprintf('%s Reading ground truth labels..\n',log_line_prefix);
                    eaf_filename = sprintf('%s\\%s.eaf',fpath, fname);

                    if ~isempty(labelmap_mat_filename)
                        load(labelmap_mat_filename,'labelmap');
                    else
                        labelmap = [];
                    end

                    viddata.ground_truth_data = ElanHelper.LoadFile(eaf_filename,viddata.fps,viddata.num_frames,Util.GROUNDTRUTH_ELAN_TIER_NAME,labelmap);

                    clear labelmap;

                    % Read train mask:
                    %       'read_trainmask' - Read train mask data from EAF files. The EAF
                    %                    file name is similar to the video filename with 
                    %                    the suffix '_train.eaf'. The tier name in the EAF file
                    %                    has to be called 'trainmask'. Segments designated for
                    %                    training should be labeled as 'Train'. Other segments
                    %                    can be unlabeled. No arguments required.
                    %viddata.train_mask = elan_load_file(eaf_filename,viddata.fps,viddata.num_frames,'trainmask',[]);

                    % Create sequence data:
                    viddata.SequenceData = SequenceData(viddata,fullname);

                    % Save matfile
                    matfilename = [fpath '\' fname suffix '.mat'];
                    save(matfilename,'viddata');
                catch exception
                    msg = sprintf('Caught exception: %s',exception.message);
                    fprintf('%s%s\n',log_line_prefix,msg);
        ...
        function viddata = LoadVidDataFromMat(fullname,varargin)
            ...
            new_var_name = sprintf('%s%s_viddata',filename,suffix);

            if do_assign_in_base
                assignin('base',new_var_name,viddata);
            end
