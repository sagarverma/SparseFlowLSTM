...
import sys
sys.path.append('/home/quadro/lrcn-caffe/python')
import caffe
import io
...
RGB_frames = '../../dataset/multimodal_FPV/multimodal_dataset/out_imgs_32x32/'
test_frames = 20
train_frames = 20
test_buffer = 25
train_buffer = 40
...
def processImageCrop(im_info, transformer, flow):
  ...
  im_flip = im_info[3]
  data_in = np.load(im_path)
  data_in = data_in.reshape((2,32,32))
  #print data_in.shape
...
class videoRead(caffe.Layer):
  ...
  def initialize(self):
    ...
    self.video_list = 'testVideos.txt' 
  ...
  def setup(self, bottom, top):
    ...
    for ix, line in enumerate(f_lines):
      video = line.split(' ')[0]
      l = int(line.split(' ')[1])
      frames = glob.glob('%s%s/*.npy' %(self.path_to_images, video))
      num_frames = len(frames)
      #print frames
      video_dict[video] = {}
      video_dict[video]['frames'] = frames[0][:-8] + '%04d.npy'
      video_dict[video]['reshape'] = (32,32)
      video_dict[video]['crop'] = (32,32)
    ...
    self.transformer.set_raw_scale('data_in', 1)
    if self.flow:
      image_mean = [0]
      ...
      image_mean = [0]
      self.transformer.set_is_flow('data_in', False)
    channel_mean = np.zeros((2,32,32))
    for channel_index, mean_val in enumerate(image_mean):
      channel_mean[channel_index, ...] = mean_val
    #self.transformer.set_mean('data_in', channel_mean)
    #self.transformer.set_channel_swap('data_in', (1, 0))
    #self.transformer.set_transpose('data_in', (0, 1))
...
class videoReadTrain_flow(videoRead):
  ...
  def initialize(self):
    ...
    self.video_list = 'trainVideos.txt' 
